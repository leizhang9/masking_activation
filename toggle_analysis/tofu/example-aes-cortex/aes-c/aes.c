...
void aes128_init(uint8_t *key)
{
    ...
    uint8_t bytes_generated = 16;
    uint8_t rcon_iteration = 1;
    uint8_t tmp[4];

    while (bytes_generated < (11 * 16))
    {

        tmp[0] = expanded_key[0 + bytes_generated - 4];
        tmp[1] = expanded_key[1 + bytes_generated - 4];
        tmp[2] = expanded_key[2 + bytes_generated - 4];
        tmp[3] = expanded_key[3 + bytes_generated - 4];
        ...
        if (bytes_generated % 16 == 0)
        {
            ...
            tmp[0] ^= round_constant[rcon_iteration];
        ...
        {
            expanded_key[bytes_generated] = expanded_key[bytes_generated - 16] ^ tmp[a];
            bytes_generated++;
        }
    }
}
...
{
    add_round_key(buffer, expanded_key, 0);
    for (uint8_t i = 1; i < 10; i++)
    {
        sub_bytes(buffer);
        shift_rows(buffer);
        mix_columns(buffer);
        add_round_key(buffer, expanded_key, i);
    }
    sub_bytes(buffer);
    shift_rows(buffer);
    add_round_key(buffer, expanded_key, 10);
}
...
void add_round_key(uint8_t *state, uint8_t *expanded_key, uint8_t round)
{
    for (uint8_t i = 0; i < 16; i++)
        state[i] = state[i] ^ expanded_key[i + (round * 16)];
}

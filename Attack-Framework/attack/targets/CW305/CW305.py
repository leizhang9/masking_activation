...
from .pll_cdce906 import PLLCDCE906
from .fpga import FPGA
from .parameter import setupSetParam
from . import util
...
class CW305(TargetTemplate):
                        ...
                        {"name": "Enabled", "key": "pllenabled", "type": "bool", "default": False, "set": self.pll.pll_enable_set, "get": self.pll.pll_enable_get, "psync": False},
                        {
                            ...
                            "children": [
                                ...
                                {"name": "CLK-SMA Slew Rate", "key": "pll0slew", "type": "list", "values": ["+3nS", "+2nS", "+1nS", "+0nS"], "default": "+0nS", "set": partial(self.pll.pll_outslew_set, outnum=0), "get": partial(self.pll.pll_outslew_get, outnum=0), "psync": False},
                                {
                                    ...
                                    "siPrefix": True,
                                    "suffix": "Hz",
                                    "set": partial(self.pll.pll_outfreq_set, outnum=0),
                                    "get": partial(self.pll.pll_outfreq_get, outnum=0),
                                    "psync": False,
                                },
                            ],
                        },
                        {
                            "name": "CLK-N13 (FGPA Pin N13)",
                            "key": "pll1",
                            "type": "group",
                            "children": [
                                ...
                                {"name": "CLK-N13 Slew Rate", "key": "pll1slew", "type": "list", "values": ["+3nS", "+2nS", "+1nS", "+0nS"], "default": "+0nS", "set": partial(self.pll.pll_outslew_set, outnum=1), "get": partial(self.pll.pll_outslew_get, outnum=1), "psync": False},
                                {
                                    ...
                                    "siPrefix": True,
                                    "suffix": "Hz",
                                    "set": partial(self.pll.pll_outfreq_set, outnum=1),
                                    "get": partial(self.pll.pll_outfreq_get, outnum=1),
                                    "psync": False,
                                },
                            ],
                        },
                        {
                            "name": "CLK-E12 (FGPA Pin E12)",
                            "key": "pll2",
                            "type": "group",
                            "children": [
                                ...
                                {"name": "CLK-E12 Slew Rate", "key": "pll2slew", "type": "list", "values": ["+0nS", "+1nS", "+2nS", "+3nS"], "default": "+0nS", "set": partial(self.pll.pll_outslew_set, outnum=2), "get": partial(self.pll.pll_outslew_get, outnum=2), "psync": False},
                                {
                                    ...
                                    "siPrefix": True,
                                    "suffix": "Hz",
                                    "set": partial(self.pll.pll_outfreq_set, outnum=2),
                                    "get": partial(self.pll.pll_outfreq_get, outnum=2),
                                    "psync": False,
                                },
                            ],
                        },
    ...
    def fpga_read(self, addr, readlen):
        ...
        if addr > self._woffset:
            _logger.info("Read from write address, confirm this is not an error")
    ...
    def gui_programfpga(self, _=None):
        ...
        if result:
            _logger.info("FPGA Config OK, time: %s" % str(stoptime - starttime))
        else:
            _logger.warning("FPGA Config failed: DONE pin did not go high. Check bitstream is for target device.")

    def _con(self, scope=None, bsfile=None, force=False, serial_number=None):
        """Connect to CW305 board, download bitstream"""

        _, sn = self._naeusb.con(idProduct=[0xC305], serial_number=serial_number)
        if self.fpga.isFPGAProgrammed() == False or force:
            ...
            if not os.path.isfile(bsfile):
                ...
                if status:
                    ...
                    _logger.warning("FPGA Done pin failed to go high, check bitstream is for target device.")

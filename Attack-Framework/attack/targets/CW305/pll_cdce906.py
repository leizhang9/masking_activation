    ...
    def outputUpdateOutputs(self, outnum, pllsrc_new=None, pllenabled_new=None, pllslewrate_new=None):
        ...
        if outnum == 0:
            ...
            if src == "PLL0":
                ...
                divsrc = 2
            # Added case if PLL settings were changed directly in the EEPROM and not via the API (by M. B. 18.09.2018)
            elif src == "UNKNOWN":
                self.parent.findParam(["pll", "pll0", "pll0source"]).setValue("PLL1")
                divsrc = 1
        elif outnum == 1:
            outpin = 1
            divsrc = 1

...
try:
    ...
    _logger.warning("PySide or PyQtGraph not installed, disabling support for pyqtgraph parameters")
...
class Parameter(object):
    ...
    def __init__(self, parent=None, ignoreChildren=False, **opts):
        ...
        name = self.opts["name"]
        if "type" not in self.opts or not isinstance(self.opts["type"], str) or self.opts["type"] not in Parameter.supportedTypes:
    ...
    def getValue(self, default=None):
        ...
        try:
            ...
            raise type(e)(type(e)(e.message + ' happens on parameter "%s"' % self.getName())).with_traceback(sys.exc_info()[2])
        ...
        if self.opts["type"] == "list":
            ...
            if isinstance(limits, dict):
                try:
                    ...
                    ValueError("Error: Value " + str(value) + ' is not valid in Parameter "' + self.getName() + '". Options are: ' + str(limits))
        return value
    ...
    def setValue(self, value, blockSignal=None, blockAction=False, init=False, ignoreReadonly=False, echo=True, addToList=False):
        ...
        if addToList:
            ...
            if (
                (type == "list" and ((isinstance(limits, dict) and value not in list(limits.values())) or (not isinstance(limits, dict) and value not in limits)))
                or (type == "bool" and value not in [True, False])
                or ((type == "int" or type == "float") and (value < limits[0] or value > limits[1]))
                or (type == "rangegraph" and (value[1] - value[0] != -1) and (value[0] < limits[0] or value[0] > limits[1] or value[1] < limits[0] or value[1] > limits[1]))
            ):
                if isinstance(limits, dict) and value in list(limits.keys()):
                    ...
                    _logger.error('Value %s out of limits (%s) in parameter "%s"' % (str(value), str(limits), self.getName()))
    ...
    def setupPyQtGraphParameter(self):
        ...
        sigSetValueAdapter = lambda v, blockSignal=None: self._PyQtGraphParameter.setValue(self.getKeyFromValue(v), sigValueUpdatedAdapter)
        ...
        if not onlyVisibles or self.isVisible():
            for child in self.childs:
                ...
                try:
                    ...
                    _logger.info("Info: Could not read parameter %s. Ignoring it..." % str(child.getPath()))
                    pass
    ...
    def load(self, fname):
        ...
        for lineNum, line in enumerate(f):
            ...
            if level != 0:
                ...
                if level >= len(path):
                    if level > len(path) + 1:
                        raise Warning("Error reading file %s, line %d. Group hierarchy missing." % (fname, lineNum))
                ...
                if child is None:
                    _logger.warning('Parameter "%s" in line %d not found. Ignoring it.' % (str(param), lineNum))
                    continue
                ...
                if child.getType() == "int":
                    ...
                    continue
                elif child.getType() == "str" or child.getType() == "text" or child.getType() == "action" or child.getType() == "file" or child.getType() == "filelist":
                    pass
                ...
                if not child.readonly():
                    ...
                    if str(child.getValue()) != str(value):
                        _logger.info("Parameter %s in line %d is readonly and value being set is different than the current one." % (child.getName(), lineNum))
    ...
    def setParameter(cls, parameter, echo=False, blockSignal=False):
        ...
        if isinstance(child.getOpts().get("values", None), dict):
            try:
                ...
                raise ValueError("Invalid value '%s' for parameter '%s'.\nValid values: %s" % (value, str(parameter), list(child.getOpts()["values"].keys())))
...
if __name__ == "__main__":
    ...
    class submodule(Parameterized):
        def __init__(self, d):
            ...
            moreparams = [{"name": "Value", "type": "list", "values": [2, 3, 4], "value": 3, "action": self.action, "linked": ["Linked Value (1/Value)"]}, {"name": "Linked Value (1/Value)", "type": "float", "default": 1 / 3, "get": self.getLinked}]
    ...
    class maintest(Parameterized):
        def __init__(self):
            ...
            p = [
                {"name": "Module", "type": "list", "values": self.values, "set": self.setmodule, "get": self.getmodule, "childmode": "root", "help": "%namehdr%Boatload of text is possible here. Can use markup too with external help window."},
            ...
            self.test.addChildren([{"name": "baud", "type": "int", "key": "baud", "limits": (500, 2000000), "value": 38400}])

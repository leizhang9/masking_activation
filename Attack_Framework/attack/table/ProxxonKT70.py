...
class Koordinatentisch:
    ...
    def getPos(self):
        ...
        # print state
        match = re.search(r"MPos:([-+]?\d*\.\d+),([-+]?\d*\.\d+),([-+]?\d*\.\d+),", state)
        x_coordinate = float(match.group(1))  # noqa: F841
        y_coordinate = float(match.group(2))  # noqa: F841
    ...
    def moveRelPos(self, x, y):
        ...
        # print command
        self.s.write(command + "\r\n")
        response = self.s.readline()  # noqa: F841
        # print response
        while 1:
            time.sleep(0.1)
            self.s.flushInput()
...
def main():
    ...
    t = Koordinatentisch("/dev/ttyACM0", 115200, 1)
    t.sendPreamble()
    # ~ #print t.getPos()
